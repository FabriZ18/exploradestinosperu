...
document.addEventListener('DOMContentLoaded', () => {
  // Código para la funcionalidad de búsqueda de paquetes turísticos
  ...
  if (searchButton && departmentSelect && durationSelect && packages.length > 0) {
  ...
  // Código para el manejo del formulario y redirección
  const currentPage = window.location.href;

  // Código para form.html
  if (currentPage.includes('form.html')) {
    ...
    if (form) {
      form.addEventListener('submit', function(event) {
        ...
        localStorage.setItem('mensajeExito', '¡Registro exitoso!');
  ...
  if (currentPage.includes('index.html')) {
    ...
    if (mensajeExito) {
      alert(mensajeExito); // Muestra el mensaje de éxito
      localStorage.removeItem('mensajeExito'); // Limpia el mensaje
    }
  }
});
